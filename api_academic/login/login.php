...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $sql1 = $conexao->prepare("SELECT codigo_contato FROM tb_contato WHERE email_contato LIKE :email");
    $sql1->bindValue(':email', $email, PDO::PARAM_STR);
    $sql1->execute();
    ...
    if($contador1 == 0){
        ...
        $sql2 = $conexao->prepare("SELECT cnpj_instituicao FROM tb_instituicao WHERE Tb_Contato_codigo_contato = :contato AND senha_instituicao LIKE :senha");
        ...
        if($contador2 == 0){
            $sql3 = $conexao->prepare("SELECT cpf_usuario FROM tb_usuario WHERE Tb_Contato_codigo_contato = :contato AND senha_usuario LIKE :senha");
